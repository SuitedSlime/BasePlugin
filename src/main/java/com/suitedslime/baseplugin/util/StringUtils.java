...
public class StringUtils {
    ...
    private static HashMap<String, Integer> stringValues = new HashMap<String, Integer>(4);
     ...
     * @return -1 if a is less then b, 1 if a is greater then b and 0 if a and b are equal.
     */
    public static int versionCompare(String a, String b) {
        ...
        String[] partsa = a.replaceAll("([0-9]+)([a-z]+)", "$1.$2").replaceAll("([a-z]+)([0-9]+)",
                "$1.$2").split("[_ ,.+\\-]{1}");
        String[] partsb = b.replaceAll("([0-9]+)([a-z]+)", "$1.$2").replaceAll("([a-z]+)([0-9]+)",
                "$1.$2").split("[_ ,.+\\-]{1}");
        ...
        for (int i = 0; i < max; ++i) {
            ...
            Integer valb = (i < partsa.length) ? numericValueOfVersionPart(partsb[i]) : 0;
